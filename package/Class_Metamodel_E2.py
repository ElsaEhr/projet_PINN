...
class MetaModel():
    ...
    def __init__(self, device, inputs, layers, E_0, E_ref, E_interpolation, n_E=10, activation=nn.Tanh(), optim="Adam",
        ...
        self.lambdas = {'res': 1, 'obs': 1, 'obs_F_u': 1,
                        'BC': 1, 'obs_F_sigma': 1, 'constitutive': 1}
    ...
    def pretrain_sigma(self, inputs, pre_train_iter=100, lambdas={'res': 1, 'obs': 0, 'obs_F': 0, 'BC': 1, 'lines': 1, 'constitutive': 1}):
    ...
    def train_model(self, inputs, dic_model, alter_steps=10,
                    ...
                    lambdas_identif_E={'res': 0, 'obs': 0, 'obs_F_u': 1,
                                       'BC': 0, 'obs_F_sigma': 0, 'constitutive': 1},
                    lambdas_update_sigma={
                        'res': 0.1, 'obs': 0, 'obs_F_u': 0, 'BC': 0.1, 'obs_F_sigma': 1, 'constitutive': 10},
                    lambdas_update_u={'res': 0, 'obs': 1, 'obs_F_u': 0, 'BC': 0, 'obs_F_sigma': 0, 'constitutive': 10}):
        """
        ...
        def J_update_u(metamodel, domain, inputs, dic_model):
            return (torch.tensor(0.),
                    ...
                    Mechanics_model.J_obs_F_u(self, inputs),
                    torch.tensor(0.),
                    torch.tensor(0.),
                    metamodel.lambdas['constitutive'] * 1/metamodel.normalized_losses['constitutive'] *
                    Mechanics_model.J_constitutive(
